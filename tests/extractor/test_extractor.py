...
def sample_pdb_file(tmp_path: Path) -> Path:
    ...
    With deduplication enabled:
      - Chain H should yield ["MET"] (adjacent duplicate removed).
    ...
    pdb_content = (
        "MODEL        1\n"
        # Chain H: First residue "MET" with residue id 1
        ...
        # Chain H: Second residue "MET" with residue id 2
        ...
        # Chain L: First residue "ALA" with residue id 1
        ...
        # Chain L: Second residue "MET" with residue id 2
...
def test_biopdb_extractor_with_deduplication(sample_pdb_file: Path):
    ...
    Expected extraction:
      - For chain H: Two MET residues (with different residue IDs) should be deduplicated to one ["MET"].
